...
{
    public class Animation_Size : IAnimation
    {
        ...
        public void MakeAnimation(FrameworkElement elm)
        {
            if (MX)
            {
                DoubleAnimation moveAnim = new DoubleAnimation(elm.Width, ToX, new Duration(TimeSpan.FromSeconds(TimeX)));
                if ((accX + decX) >= 1)
                {
                ...
                elm.BeginAnimation(FrameworkElement.WidthProperty, moveAnim);
            }
            if (MY)
            {
                DoubleAnimation moveAnim = new DoubleAnimation(elm.Height, ToY, new Duration(TimeSpan.FromSeconds(TimeY)));
                if ((accY + decY) >= 1)
                {
                    moveAnim.AccelerationRatio = 0.5;
                    moveAnim.DecelerationRatio = 0.5;
